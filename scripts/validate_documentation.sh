#!/bin/bash
# Validation script to ensure all markdown files in the docs directory are referenced in the README
...
echo "Finding all markdown files in the docs directory..."
MARKDOWN_FILES=$(find "$DOCS_DIR" -name "*.md" -not -path "*/\.*" | sort)

# Count how many files were found
TOTAL_FILES=$(echo "$MARKDOWN_FILES" | wc -l)
echo "Found $TOTAL_FILES markdown files"

# Extract all markdown links from the README that point to docs
echo "Extracting markdown links from README.md..."
README_LINKS=$(grep -o -E '\[.*\]\(docs/[^)]+\.md\)' "$README_PATH" | sed -E 's/\[.*\]\(([^)]+)\)/\1/g' | sort)
...
echo -e "\nSummary:"
echo "- $TOTAL_FILES total markdown files in docs directory"
